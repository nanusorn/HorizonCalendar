...
final class FrameProvider {
    ...
    -> CGRect
  {
    ...
    return CGRect(origin: CGPoint(x: x, y: y), size: CGSize(width: daySize.width, height: content.daysOfTheWeekHeight ?? daySize.height))
  }
  ...
  func frameOfPinnedDaysOfWeekRowBackground(yContentOffset: CGFloat) -> CGRect {
    CGRect(
      ...
      width: monthWidth,
      height: content.daysOfTheWeekHeight ?? daySize.height)
  }
  ...
  private func heightOfDaysOfTheWeekRowInMonth() -> CGFloat {
    if monthsLayout.pinDaysOfWeekToTop {
      return 0
    } else if monthsLayout.hideDaysOfWeek {
      return content.verticalDayMargin
    } else {
      return daySize.height + content.verticalDayMargin
    }
  }
